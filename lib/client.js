	...
	return {
		...
		task: function (id) {
			return {
				...
				skips: function (query, done) {
					get(root + 'tasks/' + id + '/skips', query, done);
				},

				// Create a skipped element for a task
				addSkip: function (skip, done) {
					console.log("skip, done",skip, done);
					post(root + 'tasks/' + id + '/skips', skip, done);
				},
				...
				skip: function (sid) {
					return {
						...
						get: function (query, done) {
							get(root + 'tasks/' + id + '/skips/' + sid, query, done);
						},

						// Edit a skipped element
						edit: function (edits, done) {
							post(root + 'tasks/' + id + '/skips/' + sid, edits, done);
						},

						// Remove a skipped element
						remove: function (done) {
							del(root + 'tasks/' + id + '/skips/' + sid, null, done);
						}						
